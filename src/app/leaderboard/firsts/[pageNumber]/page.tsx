...
export default async function MatchesPlayedLeaderboard({params}: {params: {pageNumber: string}}) {
    ...
    const data = await db.query.users.findMany({
        ...
        orderBy: desc(users.numberOneResults),
    });
    ...
    return (
        <div className="space-y-5">
            <Pagination>
                <PaginationContent>
                    <PaginationItem>
                        <PaginationPrevious href={pageNumber > 0 ? `/leaderboard/matches/${pageNumber - 1}` : undefined} />
                    </PaginationItem>
                    {pageNumber > 1 && (
                        <>
                            <PaginationItem>
                    ...
                    <li key={user.id}>
                        <div className="flex items-center gap-2 justify-between bg-card rounded-[var(--radius)] p-5">
                            <div className="flex items-center">
                                ...
                                <Link href={`/user/${user.name}`} className="hover:underline text-lg font-bold tracking-tight">
                                    {user.name}
                                </Link>
                            </div>
                            <div className="flex items-center gap-5">
                                <p className="text-muted-foreground">
                                    {user.numberOneResults} number one results
                                </p>
